...
public class MemberDAO {
	...
	/*
	 * public static List<MemberVO> getList(){ //인자를 받는 경우는 between 으로 서버의 일부만 받아
	 * 게시판 속도를 올리는 용도 SqlSession ss = FactoryService.getFactory().openSession();
	 * List<MemberVO> list = ss.selectList("memo.total");
	 * 
	 * ss.close();
	 * 
	 * return list; //null값은 없고 공백이 나간다.
	 * 
	 * }
	 * 
	 * 
	 * public static MemberVO getView(String i){ //인자를 받는 경우는 between 으로 서버의 일부만 받아
	 * 게시판 속도를 올리는 용도 SqlSession ss = FactoryService.getFactory().openSession();
	 * MemberVO mem = ss.selectOne("memo.view",i);//idx
	 * 
	 * ss.close();
	 * 
	 * return mem; //null값은 없고 공백이 나간다. //return ss.selectOne("memo.view",i); 이렇게하면
	 * session close를 못한다
	 * 
	 * }
	 */
